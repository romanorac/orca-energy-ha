...
REVERSE_MODE_MAPPING = {"heat_cool": 3, "cool": 2, "heat": 1, "off": 0}
...
async def async_setup_entry(hass, entry, async_add_entities, discovery_info=None):
    ...
    async_add_entities([OrcaClimate(coordinator, orca)])
...
class OrcaClimate(CoordinatorEntity, ClimateEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if floor_heating_24h_schedule == "24 hours":
            ...
            self._attr_target_temperature_high = self.coordinator.data[
                "2_Temp_prostor_dnevna"
            ].value
            self._attr_target_temperature_low = self.coordinator.data[
        ...
        if valve_status == "floor_heating":
            self._attr_hvac_action = ACTION_MAPPING.get(
                self.coordinator.data["2_Rezim_delov_TC"].value
            )

        # set based on mode only if HP is turned on
        if self.coordinator.data["2_MK1_vklop"].value == True:
            self._attr_hvac_mode = MODE_MAPPING.get(
                self.coordinator.data["2_Rezim_MK1"].value
            )
        else:
